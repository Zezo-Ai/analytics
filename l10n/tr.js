OC.L10N.register(
    "analytics",
    {
    "Version" : "Sürüm",
    "Translation error. Possibly wrong ISO code?" : "Çeviri sorunu. ISO kodu yanlış olabilir mi?",
    "Choose language" : "Dil seçin",
    "Loading" : "Yükleniyor",
    "Please select" : "Lütfen seçin",
    "Loading more..." : "Diğerleri yükleniyor...",
    "Datasources still loading. Retry" : "Veri kaynaklarının yüklenmesi sürüyor. Yeniden dene",
    "Data source not available anymore" : "Veri kaynağı artık kullanılamıyor",
    "Edit text variables" : "Yazı değişkenlerini düzenle",
    "Edit file name" : "Dosya adını düzenle",
    "File name" : "Dosya adı",
    "Keep the selected path and replace parts of the file name with text variables, for example report_%today%(Y-m-d).csv. The variable is resolved when the data load runs. See the {linkstart}Wiki{linkend}." : "Seçilen yolu koruyun ve dosya adının parçalarını yazı değişkenleriyle değiştirin. Örnek rapor_%togund%(Y-m-d).csv. Veri yükü hazırlandığında değişken çözümlenir. Ayrıntılı bilgi almak için {linkstart}Wiki{linkend} sayfasına bakabilirsiniz.",
    "Entire table" : "Tüm tablo",
    "Shared views" : "Paylaşılmış görünümler",
    "Column Picker" : "Sütun seçici",
    "Parameter missing" : "Parametre eksik",
    "Custom Column" : "Özel sütun",
    "Add custom column" : "Özel sütun ekle",
    "Select the required columns.<br>Rearrange the sequence with drag & drop.<br>Remove all selections to reset.<br>Add custom columns including text variables for dates. See the {linkstart}Wiki{linkend}." : "Gerekli sütunları seçin.<br>Sürükleyip bırakarak sırayı değiştirebilirsiniz.<br>Sıfırlamak için tüm seçimleri kaldırın.<br>Tarihler için yazı değişkenleri ile birlikte özel sütunlar ekleyin. {linkstart}Viki{linkend} sitesinden bilgi alabilirsiniz.",
    "Select file" : "Dosyayı seç",
    "Share" : "Paylaş",
    "Select the share receiver" : "Paylaşım alıcısını seçin",
    "No changes possible" : "Yapılabilecek bir değişiklik yok",
    "Add share link" : "Paylaşım bağlantısı ekle",
    "Share link" : "Paylaşım bağlantısı",
    "Link copied" : "Bağlantı kopyalandı",
    "Text variables can be used to evaluate a date value.<br>For example %today% can be used to highlight the data of today.<br>Operator and value are not relevant in this case." : "Yazı değişkenleri bir tarih değerini değerlendirmek için kullanılabilir.<br>Örneğin, bugünün verilerini vurgulamak için %today% kullanılabilir.<br>Bu durumda işlem ve değer ilgili değildir.",
    "Text variables" : "Yazı değişkenleri",
    "new record" : "yeni kayıt",
    "Missing data" : "Eksik veriler",
    "Add a report to the favorites to be shown here." : "Burada görüntülenen sık kullanılanlara bir rapor ekler.",
    "Datasets cannot be added to favorites" : "Veri kümeleri sık kullanılanlara eklenemez",
    "Settings" : "Ayarlar",
    "Data" : "Veriler",
    "Automation" : "Otomasyon",
    "The data load options could not be loaded" : "Veri yükleme seçenekleri yüklenemedi",
    "Data maintenance is not possible for this type of report" : "Bu rapor türü için veri bakımı yapılamaz",
    "Failed to load data loads" : "Veri yüklemeleri yüklenemedi",
    "Choose a data source from the dropdown and press \"+\"" : "Açılan kutudan bir veri kaynağı seçip \"+\" üzerine tıklayın",
    "Choose a data load from the list to change its settings" : "Listeden ayarlarını değiştirmek istediğiniz veri yüklemesini seçin",
    "Filter by" : "Şuna göre süz",
    "Operator" : "İşlem",
    "Value" : "Değer",
    "Please select data source" : "Lütfen veri kaynağını seçin",
    "Failed to create data load" : "Veri yükü oluşturulamadı",
    "Failed to save data load" : "Veri yüklemesi kaydedilemedi",
    "Saved" : "Kaydedildi",
    "Failed to copy data load" : "Veri yüklemesi kopyalanamadı",
    "Data load copied" : "Veri yüklemesi kopyalandı",
    "Delete data load" : "Veri yüklemesini sil",
    "Are you sure?" : "Emin misiniz?",
    "Failed to delete data load" : "Veri yüklemesi silinemedi",
    "Data source simulation" : "Veri kaynağı benzetimi",
    "Load started" : "Yükleme başlatıldı",
    "Failed to run data load" : "Veri yüklemesi çalıştırılamadı",
    "records inserted" : "kayıt eklendi",
    "records updated" : "kayıt güncellendi",
    "errors" : "hatalı",
    "deletions" : "silmeler",
    "This dataset is not used!" : "Bu veri kümesi kullanılmıyor!",
    "Delete" : "Sil",
    "All data including all reports will be deleted!" : "Tüm raporlardaki tüm veriler silinecek!",
    "The dataset name is missing" : "Veri kümesinin adı eksik",
    "Done" : "Tamam",
    "equal to" : "şuna eşit olan",
    "greater than" : "şundan büyük olan",
    "less than" : "şundan küçük olan",
    "contains" : "şunu içeren",
    "not contains" : "şunu içermeyen",
    "list of values" : "değer listesi",
    "Column selection" : "Sütun seçimi",
    "Removing columns will aggregate the key figures.<br>This applies to the chart and table" : "Sütunların kaldırılması önemli rakamları birleştirir.<br>Bu, çizelge ve tablo için geçerlidir",
    "Filter" : "Süzgeç",
    "Dynamic text variables can be used to select dates.<br>The selection is written between two % (e.g. %last2months%).<br>Use * and ? as wildcards for contains and not contains filters.<br>Information on available filters and alternative date formats is available in the {linkstart}Wiki{linkend}." : "Tarihleri seçmek için dinamik yazı değişkenleri kullanılabilir.<br>Seçim, iki % karakteri arasına yazılır (%last2months% gibi).<br>şunu içeren ve şunu içermeyen süzgeçlerinde * ve ? genel arama karakterlerini kullanabilirsiniz.<br>Kullanılabilecek süzgeçler ve alternatif tarih biçimleri hakkında {linkstart}Wiki{linkend} üzerinden ayrıntılı bilgi alabilirsiniz.",
    "Top N" : "En yüksek N",
    "none" : "yok",
    "Flop N" : "En düşük N",
    "Results can be limited to Top or Flop N. Remaining values may be summarized as \"others\"." : "Sonuçlar En yüksek veya En düşük N ile sınırlanabilir. Kalan değerler \"diğerleri\" olarak özetlenebilir.",
    "Time aggregation" : "Zaman birleştirme",
    "summation" : "toplama",
    "average" : "ortalama",
    "Aggregate daily data into weeks, months, or years" : "Günlük verileri haftalık, aylık ya da yıllık olarak birleştirin",
    "Reset table" : "Tabloyu sıfırla",
    "Table options" : "Tablo seçenekleri",
    "Configure the source layout, calculated columns, and table display settings." : "Kaynak yerleşimini, hesaplanan sütunları ve tablo görüntüleme ayarlarını yapılandırın.",
    "A pivoted table needs fields in Rows, Columns, and Measures." : "Pivot tablosunun satırlar, sütunlar ve ölçüler alanları bulunmalıdır.",
    "Untitled calculated column" : "Adsız hesaplanan sütun",
    "Calculated column" : "Hesaplanmış sütun",
    "Add" : "Topla",
    "Subtract" : "Çıkar",
    "Multiply" : "Çarp",
    "Divide" : "Böl",
    "Percentage" : "Yüzde",
    "Own Formular" : "Kendi formülünüz",
    "Unavailable column" : "Sütun kullanılamıyor",
    "Existing calculated columns could not be parsed. The current raw value stays unchanged until you save a new calculated-column configuration." : "Var olan hesaplanan sütunlar işlenemedi. Var olan ham değer, yeni bir hesaplanmış sütun yapılandırması kaydedilene kadar değişmeden kalır.",
    "No calculated columns configured" : "Herhangi bir hesaplanan sütun yapılandırılmamış",
    "{operation} using {count} source columns" : "{operation}, {count} kaynak sütun kullanıyor",
    "Column {column}" : "{column} sütunu",
    "No formula entered" : "Bir formül yazılmamış",
    "No source columns selected" : "Herhangi bir kaynak sütun seçilmemiş",
    "Add calculated column" : "Hesaplanan sütun ekle",
    "Edit calculated column" : "Hesaplanan sütunu düzenle",
    "A table column used by this calculation is not available in the selected layout." : "Bu hesaplama tarafından kullanılan bir tablo sütunu seçilmiş yerleşimde kullanılamıyor.",
    "Choose a column" : "Bir sütun seçin",
    "Select a source column to insert its reference into the formula." : "Başvuru olarak formüle eklenecek bir kaynak sütun seçin.",
    "Formula source columns are read from the formula." : "Formülün okuduğu formül kaynak sütunları.",
    "Select the numerator first and the denominator second." : "Önce payı, sonra da paydayı seçin.",
    "The first source column is the base value. All following columns are subtracted from it." : "İlk kaynak sütun temel değerdir. Aşağıdaki tüm sütunlar ondan çıkarılır.",
    "The first source column is divided by all following columns." : "İlk kaynak sütun, izleyen tüm sütunlara bölünür.",
    "All selected source columns are multiplied." : "Seçilmiş tüm kaynak sütunlar çarpılır.",
    "Select at least two source columns. Their order is preserved in the calculation." : "En az iki kaynak sütun seçin. Hesaplamada sıralara korunur.",
    "Up" : "Yukarı",
    "Down" : "Aşağı",
    "Remove" : "Kaldır",
    "Numerator" : "Pay",
    "Denominator" : "Payda",
    "Base value" : "Temel değer",
    "Subtract value" : "Çıkartılacak değer",
    "Divide by" : "Şununla böl",
    "Factor {position}" : "{position} ile çarp",
    "Source {position}" : "Kaynak {position}",
    "Choose a source column first." : "Önce bir kaynak sütun seçin.",
    "This source column is already selected." : "Bu kaynak sütun zaten seçilmiş.",
    "A percentage calculation needs exactly two source columns." : "Bir yüzde hesaplaması için iki kaynak sütun gerekir.",
    "Enter a title for the calculated column." : "Hesaplanan sütunun adını yazın.",
    "Enter a formula for the calculated column." : "Hesaplanan sütun için bir formül yazın.",
    "The formula could not be parsed." : "Formül işlenemedi.",
    "Select at least two source columns." : "En az iki kaynak sütun seçin.",
    "Sort order" : "Sıralama",
    "Default" : "Varsayılan",
    "Ascending" : "Artan",
    "Descending" : "Azalan",
    "Sort data ascending or descending" : "Verileri artan ya da azalan şekilde sıralayın",
    "Chart options" : "Çizelge seçenekleri",
    "Select the format of the data and how it should be visualized" : "Verilerin biçimini ve nasıl görselleştirileceğini seçin",
    "Data series" : "Veri dizileri",
    "Primary" : "Birincil",
    "Secondary" : "İkincil",
    "Line" : "Çizgi",
    "Bar" : "Çubuk",
    "Doughnut" : "Halka",
    "Funnel" : "Huni",
    "Thresholds" : "Eşikler",
    "Drag fields here" : "Alanları sürükleyip buraya bırakın",
    "Request could not be processed" : "İstek işlenemedi",
    "Import a csv directly; Do not use a data load." : "Doğrudan bir CSV dosyasını içe aktarın. Veri yükü kullanmayın.",
    "Favorites" : "Sık kullanılanlar",
    "Panoramas" : "Manzaralar",
    "Reports" : "Raporlar",
    "Datasets" : "Veri kümeleri",
    "New" : "Yeni",
    "New group" : "Yeni grup",
    "No panorama yet" : "Henüz bir manzara yok",
    "No report yet" : "Henüz bir rapor yok",
    "Overview" : "Özet",
    "Remove from favorites" : "Sık kullanılanlardan çıkar",
    "Add to favorites" : "Sık kullanılanlara ekle",
    "Scheduled data load" : "Zamanlanmış veri yüklemesi",
    "Delete folder" : "Klasörü sil",
    "Leaving this report?" : "Bu rapordan çıkılsın mı?",
    "You have unsaved changes. If you leave now, your changes will be lost." : "Kaydedilmemiş değişiklikleriniz var. Şimdi çıkarsanız değişiklikleriniz kaybolacak.",
    "Save" : "Kaydet",
    "Discard" : "Yok say",
    "Import failed" : "İçe aktarılamadı",
    "The panorama is not available anymore" : "Görünüm artık kullanılamıyor",
    "choose a layout" : "bir yerleşim seçin",
    "select to edit" : "düzenlemek için seçin",
    "choose the content" : "içeriği seçin",
    "Show legend" : "İşaret görüntülensin",
    "Choose a report" : "Bir rapor seçin",
    "The selected picture is not available" : "Seçilmiş görsel bulunamadı",
    "Select layout" : "Yerleşim seçin",
    "Delete current page" : "Geçerli sayfayı sil",
    "All data will be deleted!" : "Tüm veriler silinecek!",
    "Select the location for the PDF file" : "PDF dosyasının konumunu seçin",
    "Save to selected folder" : "Seçilmiş klasöre kaydet",
    "Download" : "İndir",
    "Export as PDF" : "PDF olarak dışa aktar",
    "Starting PDF export" : "PDF dışa aktarma başlatılıyor",
    "capturing page {pageCount}" : "{pageCount}. sayfa yakalanıyor",
    "captured page {pageCount}" : "{pageCount} sayfa yakalandı",
    "page {pageCount} added to pdf" : "{pageCount}. sayfa PDF dosyasına eklendi",
    "creating pdf" : "PDF oluşturuluyor",
    "created pdf" : "PDF oluşturuldu",
    "The report is not available anymore" : "Bu rapor artık kullanılamıyor",
    "Reset zoom" : "Yakınlaştırmayı sıfırla",
    "No data found" : "Herhangi bir veri bulunamadı",
    "Trend" : "Eğilim",
    "The report has a technical error" : "Raporda bir teknik sorun var",
    "Report" : "Rapor",
    "No template" : "Herhangi bir kalıp yok",
    "No templates available" : "Kullanılabilecek bir kalıp yok",
    "Text variables can be used in the name or subheader.<br>They are replaced when the report is executed." : "Yazı değişkenleri ad veya alt başlıkta kullanılabilir.<br>Rapor hazırlanırken değerleri yerleştirilir.",
    "Reports can be grouped into a folder structure." : "Raporlar bir klasör yapısında gruplanabilir.",
    "Do you want to create a new group folder?" : "Yeni bir grup klasörü oluşturmak ister misiniz?",
    "Report group" : "Rapor grubu",
    "Column descriptions are derived from the dataset and cannot be changed on a report level" : "Sütun açıklamaları veri kümesinden alınır ve rapor düzeyinde değiştirilemez",
    "Datasets can be changed in a separate maintenance" : "Veri kümeleri ayrı bir bakımda değiştirilebilir",
    "Switch to the dataset maintenance from here?" : "Veri kümesi bakımına geçmek ister misiniz?",
    "Dataset" : "Veri kümesi",
    "The report name is missing" : "Rapor adı eksik",
    "The report type selection is missing" : "Rapor türü seçimi eksik",
    "At least one visualization selection is required" : "En az bir görselleştirme seçimi yapılmalıdır",
    "Incorrect chart options" : "Çizelge seçenekleri hatalı",
    "Incorrect data options" : "Veri seçenekleri hatalı",
    "This was the last report on the dataset. Do you want to delete the dataset including all data?" : "Bu rapor, veri kümesinin son raporuydu. Tüm verilerle birlikte veri kümesini silmek istiyor musunuz?",
    "Text variables can be used in the dimensions.<br>They are replaced when the data is added." : "Yazı değişkenleri boyutlar olarak kullanılabilir.<br>Veriler hazırlanırken gerçek değerleri yerleştirilir.",
    "Delete data" : "Verileri sil",
    "Records to be deleted:" : "Silinecek kayıtlar:",
    "Technical error. Please check the logs." : "Teknik bir sorun çıktı. Lütfen günlük kayıtlarına bakın.",
    "Analytics for Nextcloud" : "Nextcloud istatistikleri",
    "Demo: Thresholds" : "Tanıtım: Eşikler",
    "Type" : "Tür",
    "Thresholds can be used to highlight data. Using the notification threshold, push messages are being sent." : "Eşikler verileri vurgulamak için kullanılabilir. Bildirim eşiği kullanılarak anında bildirimler gönderilebilir.",
    "Normal value" : "Normal değer",
    "Threshold" : "Eşik",
    "Green" : "Yeşil",
    "Notification" : "Bildirim",
    "Red" : "Kırmızı",
    "Yellow" : "Sarı",
    "Demo: Population Data" : "Tanıtım: Topluluk verileri",
    "Real-time CSV data from GitHub via the *External URL* data source." : "*Dış adres* veri kaynağı üzerinden gerçek zamanlı GitHub CSV verileri.",
    "Demo: Analytics Downloads" : "Tanıtım: Analytics indirmeler",
    "Object" : "Nesne",
    "Date" : "Tarih",
    "Real-time download statistics from GitHub." : "GitHub üzerinden gerçek zamanlı indirme istatistikleri.",
    "Demo: Finance" : "Tanıtım: Mali",
    "Segment" : "Dilim",
    "Year" : "Yıl",
    "Sales and revenue of the last years." : "Geçmiş yıllardaki satış ve karlar.",
    "New in" : "Şurada yeni",
    "View changelog" : "Değişiklik günlüğünü görüntüle",
    "OK" : "Tamam",
    "Cancel" : "İptal",
    "Open documentation" : "Belgeleri aç",
    "Show in Analytics" : "Analytics içinde görüntülensin",
    "Search" : "Ara",
    "Show _MENU_ entries" : "_MENU_ kayıtlarını görüntüle",
    "Showing _START_ to _END_ of _TOTAL_ entries" : "Toplam _TOTAL_  kayıttan _START_ ile _END_ arası görüntüleniyor",
    "Showing 0 to 0 of 0 entries" : "Toplam 0 kayıttan 0 ile 0 arası görüntüleniyor",
    "Show all" : "Tümünü görüntüle",
    "aggregate" : "birleştir",
    "others" : "diğerleri",
    "Analytics" : "Analytics",
    "You created a new report: {report}" : "Yeni bir rapor oluşturdunuz: {report}",
    "You deleted the report {report}" : "{report} raporunu sildiniz",
    "You shared the report {report}" : "{report} raporunu paylaştınız",
    "shared the report {report} with you" : "sizinle {report} raporunu paylaştı",
    "You created a new dataset: {report}" : "Yeni bir veri kümesi oluşturdunuz: {report}",
    "You deleted the dataset {report}" : "{report} veri kümesini sildiniz",
    "You shared the dataset {report}" : "{report} veri kümesini paylaştınız",
    "shared the dataset {report} with you" : "sizinle {report} veri kümesini paylaştı",
    "You created a new panorama: {report}" : "Yeni bir manzara oluşturdunuz: {report}",
    "You deleted the panorama {report}" : "{report} manzarasını sildiniz",
    "You shared the panorama {report}" : "{report} manzarasını paylaştınız",
    "shared the panorama {report} with you" : "sizinle {report} manzarasını paylaştı",
    "You have added new data to dataset {report}" : "{report} veri kümesine yeni veriler eklediniz",
    "has added new data to dataset {report}" : "{report} veri kümesine yeni veriler ekledi",
    "You have imported data in dataset {report}" : "{report} veri kümesi içine veriler aktardınız",
    "has imported data in dataset {report}" : "{report} veri kümesi içine veriler aktardı",
    "New data has been added to dataset {report} via API" : "{report} veri kümesine API ile yeni veriler eklendi",
    "New data has been added to dataset {report} via data load" : "{report} veri kümesine veri yüklemesi ile yeni veriler eklendi.",
    "<strong>Analytics</strong>: New data was added to a report" : "<strong>Analytics</strong>: Rapora yeni veriler eklendi",
    "<strong>Analytics</strong>: New reports" : "<strong>Analytics</strong>: Yeni raporlar",
    "Visualize data in Analytics" : "Verilerinizi Analytics içinde görselleştirin",
    "No valid timestamp found in datasource filename" : "Veri kaynağı dosya adında geçerli bir zaman damgası bulunamadı",
    "File not found: %s" : "Dosya bulunamadı: %s",
    "Total" : "Toplam",
    "External" : "Dış",
    "External URL" : "Dış adres",
    "Header row" : "Başlık satırı",
    "Yes" : "Evet",
    "No" : "Hayıt",
    "Ignore leading rows" : "Üst satır yok sayılsın",
    "Number of rows" : "Satır sayısı",
    "Select columns" : "Sütunları seçin",
    "e.g. 1,2,4 or leave empty" : "Örnek: 1,2,4 ya da boş bırakın",
    "HTTP method" : "HTTP yöntemi",
    "Object path" : "Nesne yolu",
    "More options" : "Diğer seçenekler",
    "Authentication" : "Kimlik doğrulama",
    "Timestamp of data load" : "Veri yüklemesinin zaman damgası",
    "Releases" : "Sürümler",
    "Packages" : "Paketler",
    "Issues" : "Sorunlar",
    "Pull Requests" : "Çekme istekleri",
    "Limit" : "Sınır",
    "Filter release assets via extension" : "Eklentiye göre sürüm varlıklarını süz",
    "Show assets" : "Varlıkları görüntüle",
    "Personal access token" : "Kişisel erişim kodu",
    "optional" : "isteğe bağlı",
    "Asset" : "Varlık",
    "Download count" : "İndirilme sayısı",
    "Tag" : "Etiket",
    "Count" : "Sayı",
    "Missing or invalid GitHub access token" : "GitHub erişim kodu eksik ya da geçersiz",
    "Rate limit exceeded" : "Hız sınırlaması değeri aşıldı",
    "Report not available" : "Rapor kullanılamıyor",
    "Local" : "Yerel",
    "File" : "Dosya",
    "Current time" : "Geçerli saat",
    "From filename" : "Dosya adından",
    "Sheet" : "Sayfa",
    "sheet name" : "sayfa  adı",
    "Cell range" : "Hücre aralığı",
    "e.g. A1:C3,A5:C5" : "Örnek: A1:C3,A5:C5",
    "Invalid spreadsheet range" : "Elektronik tablo aralığı geçersiz",
    "Spreadsheet range is too large" : "Elektronik tablo aralığı çok geniş",
    "HTML grabber" : "Html yakalayıcı",
    "valid regex" : "geçerli kurallı ifade",
    "Import data into an existing dataset" : "Var olan bir veri kümesi içine veri aktar",
    "Announcements" : "Duyurular",
    "Report '{report}': {subject} reached the threshold of {rule} {value}" : "'{report}' raporu: {subject}, {rule} {value} eşiğine ulaştı",
    "Error during data load \"{dataloadName}\" for data set \"{datasetName}\"" : "\"{datasetName}\" veri kümesi için \"{dataloadName}\" verileri yüklenirken sorun çıktı",
    "Panorama" : "Manzara",
    "Report not found" : "Rapor bulunamadı",
    "This report was removed or is not shared with you" : "Bu rapor silinmiş ya da sizinle paylaşılmıyor",
    "3rd field must be a valid number" : "3. alan geçerli bir sayı olmalıdır",
    "No data" : "Herhangi bir veri yok",
    "Last field must be a valid number" : "Son alanı geçerli bir sayı olmalıdır",
    "copy" : "kopyala",
    "Reports, datasets and panoramas" : "Raporlar, veri kümeleri ve görünümler",
    "extract / store / visualize" : "ayıkla / kaydet / görselleştir",
    "Connect data from different sources in one place and turn it into shareable insights. Analytics lets teams build flexible panoramas, dashboards, and reports with seamless Nextcloud integration.\n\n## Why Analytics\n- **Centralize data**: combine CSV, spreadsheets, APIs, and Nextcloud apps in one place.\n- **Analyze faster**: use charts, tables, filters, and panoramas to explore data quickly.\n- **Share securely**: collaborate with users, groups, or share links with fine-grained access.\n- **Automate updates**: schedule data loads and keep reports current.\n\n#### Features:\n- **Visualization**: panoramas, reports, charts, tables, filters\n- **Data sources**:\n  - CSV, spreadsheets, API, imports, manual entry\n  - External sources: GitHub, website grabber, JSON\n- **Extensible**: register custom data sources from other apps\n- **Storage**: real-time or database\n- **Automation**: scheduled data loads\n- **Functions**: trend calculations, dis-/aggregation\n- **Alerts**: thresholds for notifications and color coding\n- **Integration**: Files, Activity, Notifications, Flow, Dashboard, Search, Smart picker, Translation\n- **AI Assistant**: Context Chat integration" : "Farklı kaynaklardan gelen verileri tek bir yerde birleştirin ve paylaşılabilir içgörülere dönüştürün. İstatistikler, sorunsuz Nextcloud bütünleştirmesi ile ekiplerin esnek görünüşler, panolar ve raporlar oluşturmasını sağlar.\n\n## İstatistiklerin yararları\n- **Verileri merkezileştirin**: CSV, elektronik tablo, API ve Nextcloud uygulamalarını tek bir yerde birleştirin.\n- **Daha hızlı görün**: Verileri hızlı bir şekilde keşfetmek için çizelgeleri, tabloları, süzgeçleri ve görünüşleri kullanın.\n- **Güvenli bir şekilde paylaşın**: Kullanıcılarla, gruplarla iş birliği yapın veya ayrıntılı erişimle bağlantıları paylaşın.\n- **Güncellemeleri otomatikleştirin**: Verileri zamanlayın ve raporları güncel tutun.\n\n#### Özellikler:\n- **Görselleştirme**: Görünüşler, raporlar, çizelgeler, tablolar, süzgeçler\n- **Veri kaynakları**:\n- CSV, elektronik tablo, API, içe aktarma, el ile giriş\n- Dış kaynaklar: GitHub, internet sitesi derleyici, JSON\n- **Genişletilebilir**: Diğer uygulamalardan özel veri kaynakları oluşturma\n- **Depolama**: Gerçek zamanlı veya veri tabanı\n- **Otomasyon**: Zamanlanmış veriler\n- **Fonksiyonlar**: Eğilim hesaplamaları, dis-/toplama\n- **Uyarılar**: Bildirimler ve renk kodlaması için eşikler\n- **Bütünleştirme**: Dosyalar, Etkinlik, Bildirimler, Akış, Pano, Arama, Akıllı seçici, Çeviri uygulamaları\n- **Yapay zeka yardımcısı**: Bağlam sohbeti bütünleştirmesi",
    "This report is password protected" : "Bu rapor parola korumalı",
    "The password is wrong. Try again." : "Parola yanlış. Yeniden deneyin.",
    "Password" : "Parola",
    "Javascript issue" : "JavaScript sorunu",
    "If you see this message, please disable AdBlock/uBlock for this domain (only)." : "Bu iletiyi görürseniz, lütfen (yalnızca) bu etki alanı için AdBlock/uBlock uygulamasını kullanımdan kaldırın.",
    "The EasyPrivacy list is blocking some scripts because of a wildcard filter for *analytics*." : "EasyPrivacy listesi, *analytics* için bir genel arama karakteri süzgeci nedeniyle bazı komut dosyalarını engelliyor.",
    "More Information …" : "Ayrıntılı bilgi alın…",
    "Quickstart" : "Hızlı başlangıç",
    "Intro" : "Tanıtım",
    "Wiki" : "Wiki",
    "Options" : "Seçenekler",
    "Show top N items and group others together" : "En yüksek N öge görüntülensin ve diğerleri gruplansın",
    "Maintain thresholds for notifications" : "Bildirimlerin eşikleri korunsun",
    "Analysis" : "İnceleme",
    "Aggregate values" : "Değer birleştirme",
    "Disaggregate values" : "Değer ayrıştırma",
    "Translate" : "Çevir",
    "Auto refresh" : "Otomatik yenileme",
    "1 min" : "1 dakika",
    "10 min" : "10 dakika",
    "30 min" : "30 dakika",
    "Download chart" : "Çizelgeyi indir",
    "Save as new report" : "Yeni rapor olarak kaydet",
    "Edit" : "Düzenle",
    "Change layout" : "Yerleşimi değiştir",
    "Report was changed - Press here to save the current state" : "Rapor değiştirildi. Geçerli durumu kaydetmek için buraya tıklayın",
    "Legend" : "İşaret",
    "Do you like this app?" : "Bu uygulamayı beğendiniz mi?",
    "Introduction" : "Giriş",
    "More information …" : "Ayrıntılı bilgi alın…",
    "Name" : "Ad",
    "Variables" : "Değişkenler",
    "Subheader" : "Alt başlık",
    "Data source" : "Veri kaynağı",
    "Saved Data" : "Kaydedilmiş veriler",
    "Data source options" : "Veri kaynağı seçenekleri",
    "Column headers" : "Sütun başlıkları",
    "Column" : "Sütun",
    "Visualization" : "Görselleştirme",
    "Display" : "Görünüm",
    "Chart & Table" : "Çizelge ve tablo",
    "Table" : "Tablo",
    "Chart" : "Çizelge",
    "Chart type" : "Çizelge türü",
    "Timeline (date in column 2)" : "Zaman tüneli (tarih 2. sütunda)",
    "Bar - Stacked" : "Çubuk - Yığılmış",
    "Bar - Stacked 100%%" : "Çubuk - Yığılmış %%100",
    "Area" : "Alan",
    "Area - 100%%" : "Alan - %%100",
    "Data options" : "Veri seçenekleri",
    "Export" : "Dışa aktar",
    "Update" : "Güncelle",
    "Time dimension" : "Zaman boyutu",
    "Grouping" : "Gruplama",
    "Mode" : "Kip",
    "Group by" : "Grupla",
    "Option" : "Seçenek",
    "with others" : "diğerleri ile",
    "Select the source columns available to the chart and table." : "Çizelge ve tabloda kullanılabilecek kaynak sütunları seçin.",
    "Visualizations" : "Görselleştirmeler",
    "Add filter" : "Süzgeç ekle",
    "Remove filter" : "Süzgeci kaldır",
    "Number of records" : "Kayıt sayısı",
    "Nextcloud Assistant" : "Nextcloud Yardımcısı",
    "Context Chat is required" : "Context Chat gereklidir",
    "Description" : "Açıklama",
    "Explain the data and the purpose of the data in a full sentence. This is required for the AI Assistant to understand the raw data for further processing." : "Verileri ve verilerin amacını bir cümleyle açıklayın. Bu açıklama, Yapay Zeka Yardımcı uygulamasının ham verileri daha iyi işlemeyi anlaması için gereklidir.",
    "Context Chat relevant" : "Context Chat ile ilgili",
    "Manual index update" : "El ile dizin güncellemesi",
    "Manual entry" : "El ile giriş",
    "Save data" : "Verileri kaydet",
    "Import" : "İçe aktar",
    "From file" : "Dosyadan",
    "From clipboard" : "Panodan",
    "REST API" : "REST API",
    "Use this endpoint to submit data via an API:" : "Bir API ile veri göndermek için bu uç noktayı kullanın:",
    "Data load" : "Veri yüklemesi",
    "Advanced configuration" : "Gelişmiş yapılandırma",
    "Thresholds can trigger notifications and color coding in reports." : "Eşikler bildirimleri ve raporlardaki renk kodlamalarını belirler.",
    "Severity" : "Ağırlık",
    "Coloring in table" : "Tablo renklendirmesi",
    "Single value" : "Tek değer",
    "Table row" : "Tablo satırı",
    "Notification for new records" : "Yeni kayıtlar için bildirim",
    "Save threshold" : "Eşiği kaydet",
    "Existing thresholds will be evaluated in the order shown below. The order can be changed by drag & drop." : "Var olan eşikler aşağıda gösterilen sırayla değerlendirilir. Sıralama sürükleyip bırakarak değiştirilebilir.",
    "Selection" : "Seçim",
    "Data deletion" : "Veri silme",
    "New deletion" : "Yeni silme",
    "Setting" : "Ayar",
    "Title" : "Başlık",
    "Delete data before load" : "Yüklemeden önce veriler silinsin",
    "Copy" : "Kopyala",
    "Execution" : "Yürütme",
    "Load now" : "Şimdi yükle",
    "Test run" : "Deneme çalışması",
    "Schedule in background" : "Arka planda zamanla",
    "Not scheduled" : "Zamanlanmamış",
    "Daily" : "Günlük",
    "Daily - End of day" : "Günlük - Gün sonunda",
    "Daily - Start of day" : "Günlük - Gün başında",
    "Hourly" : "Saatlik",
    "Load via OCC command:" : "OCC komutu ile yükle:",
    "Import report" : "Raporu içe aktar",
    "Basic settings" : "Temel ayarlar",
    "Rename" : "Yeniden adlandır",
    "Add to new group" : "Yeni gruba ekle",
    "Dataset maintenance" : "Veri kümesi bakımı",
    "Unshare" : "Paylaşımdan kaldır",
    "can navigate" : "gezinebilir",
    "Password protection" : "Parola koruması",
    "Share chart for external website" : "Dış site için çizelge paylaşımı",
    "To including a chart in an external website, that domain needs to be allowed (CSP policy)" : "Dışarıdaki bir siteye bir çizelge eklemek için bu etki alanına izin verilmelidir (CSP ilkesi)",
    "Copy link" : "Bağlantıyı kopyala",
    "Domain of external site" : "Dış sitenin etki alanı",
    "Data Source" : "Veri kaynağı",
    "Position the available fields to define the structure of the table. For a classic list view, move all fields into rows." : "Tablonun yapısını tanımlamak için var olan alanları konumlandırın. Klasik bir liste görünümü için tüm alanları satırlara taşıyın.",
    "Rows" : "Satırlar",
    "Columns" : "Sütunlar",
    "Measures" : "Ölçümler",
    "Not required" : "Zorunlu değil",
    "Calculated Columns" : "Hesaplanan sütunlar",
    "Close" : "Kapat",
    "Calculation method" : "Hesaplama yöntemi",
    "Available columns" : "Kullanılabilecek sütunlar",
    "Add source column" : "Kaynak sütunu ekle",
    "Formula" : "Formül",
    "Example: (30 * [Quantity]) + ([Price] * -1)" : "Örnek: (30 * [Adet]) + ([Fiyat] * -1)",
    "Use the displayed column names, fixed numbers, +, -, *, /, parentheses, and ROUND(value, decimals)." : "Görüntülenen sütun adlarını, sabit sayıları, +, -, *, /, parantezleri ve YUVARLAK (değer, ondalık) kullanın.",
    "Remove calculated column" : "Hesaplanan sütunu kaldır",
    "Show totals" : "Toplamlar görüntülensin",
    "Format all numbers in local format" : "Tüm sayılar yerel ayarlara göre biçimlendirilsin",
    "Compact visualization" : "Dar görselleştirme",
    "Data format" : "Veri biçimi",
    "Select how the raw data is structured" : "Ham verilerin nasıl yapılandırıldığını seçin",
    "In rows" : "Satırlar biçiminde",
    "Data series are organized by horizontal rows." : "Veri kümeleri yatay satırlar olarak düzenlenir.",
    "In columns" : "Sütunlar biçiminde",
    "Data series are grouped by vertical columns." : "Veri kümeleri dikey satırlar olarak düzenlenir.",
    "Timestamps" : "Zaman damgaları",
    "Use first column for time-series indexing." : "Zaman serisi dizini olarak ilk sütun kullanılır.",
    "Labels" : "Etiketler",
    "Choose how doughnut chart labels are displayed" : "Halka çizelgelerin nasıl görüntüleneceğini seçin",
    "Doughnut label style" : "Halka etiketi biçemi",
    "Absolute value" : "Mutlak değer",
    "Vertical axis" : "Dikey eksen",
    "Sort by" : "Sıralama",
    "Direction" : "Yön",
    "Nextcloud Analytics makes your data visible and helps you to evaluate them - from financial data to IoT logs." : "Nextcloud Analytics, mali verilerden IoT günlüklerine kadar çeşitli verilerinizi görünür kılar ve  bunları değerlendirmenize yardımcı olur.",
    "Give your numbers a meaning." : "Rakamlarınızı anlamlandırın.",
    "Data is processed inside Nextcloud." : "Veriler Nextcloud içinde işlenir.",
    "Share your reports and insights" : "Raporlarınızı ve öngörülerinizi paylaşın",
    "Fully integrated into Nextcloud" : "Nextcloud ile tam olarak bütünleştirilmiş",
    "Open Source" : "Açık kaynaklı",
    "Visualize any data with ease" : "Herhangi bir veriyi kolayca görselleştirin",
    "Showcase key insights with Panoramas" : "Manzaralar ile önemli içgörüleri sergileyin",
    "Various chart types, including lines, columns, and doughnuts" : "Çizgi, sütun ya da halka gibi çeşitli çizelge türleri",
    "Gain a comprehensive view of your data with Nextcloud Dashboard" : "Nextcloud Pano ile verilerinizin kapsamlı bir görünümünü elde edin",
    "Interactive tables for deeper analysis" : "Daha derin incelemeler için etkileşimli tablolar",
    "Set thresholds to highlight exceptions and receive Nextcloud notifications" : "Sıradışı durumları vurgulamak ve Nextcloud bildirimleri almak için eşikleri ayarlayın",
    "Slice and dice your data with precision" : "Verilerinizi incelikle ayrıştırıp ve parçalara ayırın",
    "Apply flexible filters, including dynamic date variables" : "Devingen tarih değişkenleri ile esnek süzgeçler uygulayın",
    "Top N, sorting and time aggregation" : "En yüksek N, sıralama ve zaman birleştirme",
    "Customize charts with primary and secondary axes" : "Birincil ve ikincil eksenler ile çizelgeleri özelleştirin",
    "Mix and match chart types for each data series" : "Her veri dizisi için çizelge türlerini karıştırın ve eşleştirin",
    "Save default filters for quick access to your preferred views" : "Yeğlediğiniz görünümlere hızlı erişmek için varsayılan süzgeçler kaydedin",
    "Utilize data from multiple sources with ease" : "Birden fazla kaynaktan gelen verileri kolaylıkla kullanın",
    "Data is persisted in the database for maximum flexibility and performance" : "Veriler, en iyi esneklik ve başarım için veri tabanında tutulur",
    "Monitor real-time download statistics with GitHub integration" : "GitHub bütünleştirmesiyle gerçek zamanlı indirme istatistiklerini izleyin",
    "Leverage the JSON data source to pull data from external services" : "Dış hizmetlerdeki verileri çekmek için JSON veri kaynağını kullanın",
    "Extract data from virtually any website using the HTML-Grabber" : "HTML yakalayıcı ile hemen hemen her siteden veriler çıkarın",
    "Visualize data from internal Nextcloud applications instantly" : "İç Nextcloud uygulamalarından gelen verileri anında görselleştirin",
    "Automate your data workflows seamlessly" : "Veri iş akışlarınızı kolayca otomatikleştirin",
    "Persist data from any source directly in Nextcloud" : "Herhangi bir kaynaktan gelen verileri doğrudan Nextcloud üzerinde tutun",
    "Utilize timestamps to create historical records" : "Zaman damgalarını kullanarak geçmiş kayıtları oluşturun",
    "Perform full or incremental delta loads" : "Tam ya da artımlı fark yüklemeleri yapın",
    "Schedule automatic data loads in the background" : "Arka planda otomatik veri yüklemeleri zamanlayın",
    "Trigger data loads with scripts using the occ command" : "Veri yüklemelerini occ komutu kullanan betiklerle tetikleyin",
    "Push data into Analytics effortlessly via the REST API" : "REST API ile verileri kolayca İstatistikler üzerine aktarın",
    "Get more information" : "Ayrıntılı bilgi alın",
    "Nextcloud forum" : "Nextcloud forum",
    "You can open this introduction again by selecting \"Introduction\" in the Analytics settings section." : "Bu tanıtımı daha sonra, Analytics ayarları bölümündeki \"Giriş\" seçeneğini ile açabilirsiniz.",
    "Activate a set of demo report to show how Analytics works." : "Analytics çalışmasını görmek için bazı tanıtım raporlarını kullanıma alın.",
    "Create Demo" : "Tanıtımı hazırla",
    "Start using Analytics" : "Analytics kullanmaya başlayın",
    "Own report" : "Kendi raporunuz",
    "Start from scratch and configure all report settings manually." : "Sıfırdan başlayın ve tüm rapor ayarlarını el ile yapılandırın.",
    "Templates" : "Kalıplar",
    "General information" : "Genel bilgiler",
    "Report title" : "Rapor başlığı",
    "New report" : "Yeni rapor",
    "Text variables can be used in the name or subheader. They are replaced when the report is executed." : "Yazı değişkenleri ad veya alt başlıkta kullanılabilir. Rapor hazırlanırken değerleri yerleştirilir.",
    "The following variables are available:" : "Şu değişkenler kullanılabilir:",
    "Type of report" : "Rapor türü",
    "Real-time Data" : "Gerçek zamanlı veriler",
    "Stored Data" : "Kaydedilmiş veriler",
    "Reports can read their data either from real-time data sources or from saved datasets within the Nextcloud database." : "Raporlar, verilerini gerçek zamanlı veri kaynaklarından veya Nextcloud veri tabanındaki kayıtlı veri kümelerinden okuyabilir.",
    "Different data sources are available. The data is read in real-time when the report is being executed." : "Farklı veri kaynakları kullanılabilir. Rapor hazırlanırken veriler gerçek zamanlı olarak okunur.",
    "Report template (optional)" : "Rapor kalıbı (isteğe bağlı)",
    "Templates provide preconfigured reports for a data source." : "Kalıplar bir veri kaynağı için hazır raporlar sağlar.",
    "Every data source requires specific parameter. Please enter the information." : "Her veri kaynağı için belirli bir parametre gerekir. Lütfen bilgileri yazın.",
    "New dataset" : "Yeni veri kümesi",
    "Existing dataset" : "Var olan veri kümesi",
    "The report can be based on a new dataset or read existing data that is available already." : "Rapor, yeni bir veri kümesini ya da zaten var olan verileri kullanabilir.",
    "Many reports can be based on the same data, but every report has its own visualization, threshold and sharing settings." : "Farklı raporlar aynı verileri kullanabilir, ancak her raporun kendi görselleştirme, eşik ve paylaşım ayarları bulunur.",
    "Datasets can store data from scheduled data loads, the REST API or manual data entry. This can be configured via the Dataset maintenance" : "Veri kümelerinde, zamanlanmış veri yüklemelerinden, REST API üzerinden ya da el ile yapılan veri girişinden gelen veriler bulunabilir. Bu seçenekler, veri kümesi bakımı ile yapılandırılabilir.",
    "Choose the names of the columns for the new dataset" : "Yeni veri kümesinin sütun adlarını seçin",
    "Visualization of the report" : "Rapor görselleştirmesi",
    "All settings can be changed afterwards" : "Tüm ayarlar daha sonra değiştirilebilir",
    "Visualization is preconfigured by the selected template and cannot be changed in this step." : "Görselleştirme, seçilmiş kalıp tarafından önceden yapılandırılmıştır ve bu adımda değiştirilemez.",
    "No Chart" : "Çizelge yok",
    "No Table" : "Tablo yok",
    "The report can now be created" : "Artık rapor oluşturulabilir",
    "Create" : "Oluştur"
},
"nplurals=2; plural=(n > 1);");
