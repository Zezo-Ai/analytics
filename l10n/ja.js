OC.L10N.register(
    "analytics",
    {
    "Version" : "バージョン",
    "Translation error. Possibly wrong ISO code?" : "翻訳エラー。ISOコードを確認。",
    "Choose language" : "言語を選択",
    "Loading" : "読み込み中",
    "Please select" : "選択してください",
    "Loading more..." : "さらに読み込む",
    "Data source not available anymore" : "データソースが利用できなくなりました",
    "Edit file name" : "ファイル名を編集",
    "File name" : "ファイル名",
    "Parameter missing" : "パラメーターがない",
    "Add custom column" : "カスタム列の追加",
    "Select file" : "ファイルを選択",
    "Share" : "共有",
    "Select the share receiver" : "共有の受信者を選択してください",
    "No changes possible" : "変更はできない",
    "Add share link" : "共有リンクを追加",
    "Share link" : "リンクを共有",
    "Link copied" : "リンクをコピーしました",
    "Add a report to the favorites to be shown here." : "ここに表示されるレポートをお気に入りに追加する。",
    "Settings" : "設定",
    "Data" : "データ",
    "Automation" : "オートメーション",
    "Data maintenance is not possible for this type of report" : "このタイプのレポートでは、データのメンテナンスができません。",
    "Choose a data source from the dropdown and press \"+\"" : "ドロップダウンメニューからデータソースを選択し、\"+\"を押してください。",
    "Choose a data load from the list to change its settings" : "リストからデータロードを選択し、設定を変更します。",
    "Filter by" : "絞り込み",
    "Operator" : "条件",
    "Value" : "値",
    "Please select data source" : "データソースを選択してください。",
    "Saved" : "保存しました",
    "Data load copied" : "データロードをコピーした",
    "Delete data load" : "データロードの削除",
    "Are you sure?" : "本当に実行しますか？",
    "Data source simulation" : "データソースシミュレーション",
    "Load started" : "ロード開始",
    "records inserted" : "挿入済みレコード",
    "records updated" : "更新履歴",
    "errors" : "エラー",
    "deletions" : "削除されたもの",
    "This dataset is not used!" : "このデータセットは使用されていませんこのデータセットは使用されていません",
    "Delete" : "削除",
    "All data including all reports will be deleted!" : "すべてのレポートを含むすべてのデータが削除されます",
    "The dataset name is missing" : "データセット名がありません",
    "Done" : "完了",
    "equal to" : "同様",
    "greater than" : "より大きい",
    "less than" : "未満",
    "contains" : "を含む",
    "list of values" : "かちたいけい",
    "Filter" : "フィルター",
    "Top N" : "上位N個",
    "none" : "なし",
    "average" : "平均",
    "Aggregate daily data into weeks, months, or years" : "１日ごとのデータを週、月または年単位に集計する<br>",
    "Reset table" : "テーブルをリセット",
    "Add" : "追加",
    "Remove" : "削除",
    "Sort order" : "ソート順",
    "Default" : "デフォルト",
    "Ascending" : "昇順",
    "Descending" : "降順",
    "Sort data ascending or descending" : "データを昇順または降順で並べ替え",
    "Data series" : "データシリーズ",
    "Primary" : "プライマリ",
    "Secondary" : "セカンダリ",
    "Line" : "線",
    "Bar" : "バー",
    "Doughnut" : "ドーナツ",
    "Funnel" : "じょうご",
    "Drag fields here" : "ここにフィールドをドラッグ",
    "Request could not be processed" : "リクエストを処理できませんでした",
    "Favorites" : "お気に入り",
    "Reports" : "報告",
    "New" : "新規作成",
    "New group" : "新規グループ",
    "Overview" : "概要",
    "Remove from favorites" : "お気に入りから削除",
    "Add to favorites" : "お気に入りに追加",
    "Delete folder" : "フォルダーを削除",
    "Save" : "保存する",
    "Discard" : "破棄",
    "choose a layout" : "レイアウトを選択",
    "choose the content" : "コンテンツを選択",
    "Download" : "ダウンロード",
    "Export as PDF" : "PDFにエクスポート",
    "Starting PDF export" : "PDFにエクスポート開始",
    "Reset zoom" : "リセットズーム",
    "No data found" : "ダーターが見つかりません",
    "Trend" : "トレンド",
    "Report" : "報告",
    "Dataset" : "データセット",
    "Delete data" : "データを削除",
    "Technical error. Please check the logs." : "テクニカルエラーです。ログを確認してください。",
    "Type" : "タイプ",
    "Green" : "緑",
    "Notification" : "通知",
    "Red" : "赤",
    "Yellow" : "黄",
    "Date" : "日付",
    "Year" : "年",
    "New in" : "新機能",
    "View changelog" : "変更履歴を確認",
    "OK" : "OK",
    "Cancel" : "キャンセル",
    "Open documentation" : "ドキュメントを開く",
    "Search" : "検索",
    "Show _MENU_ entries" : "_MENU_の項目を表示する",
    "Showing _START_ to _END_ of _TOTAL_ entries" : "_TOTAL_件中_START_件までの _END_",
    "Showing 0 to 0 of 0 entries" : "0件中0件から0件を表示",
    "Show all" : "全て表示",
    "Total" : "合計",
    "External" : "外部",
    "External URL" : "外部URL",
    "Yes" : "はい",
    "No" : "いいえ",
    "Select columns" : "列を選択する",
    "e.g. 1,2,4 or leave empty" : "例：1,2,4または空白のままにする",
    "HTTP method" : "HTTPメソッド",
    "More options" : "追加のオプション",
    "Authentication" : "認証",
    "Timestamp of data load" : "データ読み込み時のタイムスタンプ",
    "Releases" : "リリース",
    "Issues" : "問題",
    "Limit" : "制限",
    "Tag" : "タグ",
    "Local" : "ローカル",
    "File" : "ファイル",
    "Current time" : "現在時刻",
    "Sheet" : "シート",
    "sheet name" : "シート名",
    "Cell range" : "セル範囲",
    "e.g. A1:C3,A5:C5" : "例：A1:C3,A5:C5",
    "HTML grabber" : "HTML grabber",
    "valid regex" : "有効な正規表現",
    "Announcements" : "お知らせ",
    "Report not found" : "レポートが見つかりません",
    "No data" : "データなし",
    "copy" : "コピーする",
    "The password is wrong. Try again." : "パスワードが違います。再入力してください。",
    "Password" : "パスワード",
    "Options" : "オプション",
    "Translate" : "翻訳",
    "1 min" : "1分",
    "30 min" : "30分",
    "Edit" : "編集",
    "Legend" : "凡例",
    "Do you like this app?" : "このアプリを気に入っていだたけましたか？",
    "Introduction" : "イントロダクション",
    "More information …" : "詳細情報",
    "Name" : "名前",
    "Variables" : "変数",
    "Subheader" : "小見出し",
    "Data source" : "データソース",
    "Column" : "カラム",
    "Display" : "表示",
    "Table" : "表",
    "Chart" : "チャート",
    "Export" : "エクスポート",
    "Update" : "更新",
    "Mode" : "モード",
    "Option" : "Option",
    "Remove filter" : "フィルターを削除",
    "Nextcloud Assistant" : "Nextcloud アシスタント",
    "Context Chat is required" : "コンテキストチャットが必要です",
    "Description" : "説明",
    "Context Chat relevant" : "コンテキストチャット関連",
    "Save data" : "データを保存",
    "Import" : "インポート",
    "REST API" : "REST API",
    "Data load" : "データロード",
    "Advanced configuration" : "高度な設定",
    "Selection" : "選択",
    "Setting" : "設定",
    "Title" : "タイトル",
    "Copy" : "コピー",
    "Daily" : "毎日",
    "Hourly" : "1時間ごと",
    "Import report" : "レポートをインポート",
    "Basic settings" : "基本設定",
    "Rename" : "名前を変更",
    "Unshare" : "共有を解除",
    "Password protection" : "パスワード保護",
    "Copy link" : "リンクをコピー",
    "Rows" : "行",
    "Columns" : "カラム",
    "Close" : "閉じる",
    "Sort by" : "ソート: ",
    "Direction" : "方向",
    "Give your numbers a meaning." : "数字に意味を持たせましょう。",
    "Data is processed inside Nextcloud." : "データはNextcloud内で処理されます。",
    "Share your reports and insights" : "レポートと洞察を共有する",
    "Open Source" : "オープンソース",
    "Visualize any data with ease" : "あらゆるデータを簡単に視覚化",
    "Interactive tables for deeper analysis" : "より深い分析のためのインタラクティブな表",
    "Slice and dice your data with precision" : "データを正確に細分化する",
    "Get more information" : "詳細情報の取得",
    "Nextcloud forum" : "Nextcloudフォーラム",
    "Templates" : "テンプレート",
    "New report" : "新しいレポート",
    "New dataset" : "新しいデータセット",
    "Create" : "作成"
},
"nplurals=1; plural=0;");
